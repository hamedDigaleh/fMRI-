...
load('totalHistogram_ohne_vor_verarbeitung.mat')
...
t=abs(t);
parfor i=0:(num_selected_peaks-1)
    B = sort(t);
    ...
    max_f(i+1)=B(1,end-i);
    ll=find(t==max_f(i+1));
    finded_positions(i+1)=find(ll(1,1));
end
 %%
...
parfor i=1:length(finded_positions)
...
for k=1:size(Datas,1)
    ...
    species = svmclassify(svmStruct,ind);
    %
