...
function [specificity precision accuracy error  sensitivity]=PerformSVM(max_num_ICs,max_num_selected_peaks,histogram,kernal_function_GUI,feature_selection_method_GUI,rbf_sigma_GUI,poloynomial_order_GUI,mlp_scale_GUI,C_value_svm)
tic
...
[feat,stat] = rankfeatures(Y,grp,'CRITERION',feature_selection_method_GUI,'NUMBER',max_num_selected_peaks);
...
switch m 
    case 1
            svmStruct = svmtrain(Datas_mod,Classes_mod,'kernel_function',kernal_function_GUI,'rbf_sigma',rbf_sigma_GUI,'BoxConstraint',C_value_svm,'method','SMO');
    species = svmclassify(svmStruct,ind(1,1:size(ind,2)));
    case 2
...
parfor i=1:146
 ...
 sensitivity(1,num_ICs) = ((TP)/(TP+FN))*100;
...
specificity(1,num_ICs) = ((TN) / (TN+FP))*100;
precision(1,num_ICs) = ((TP)/(TP+FP))*100;
accuracy(1,num_ICs) = ((TP+TN)/(TP+TN+FP+FN))*100;
error(1 ,num_ICs) =((FP+FN)/(TP+TN+FP+FN))*100;

result_matrix(:,1)=accuracy';
result_matrix(:,2)=precision';
result_matrix(:,3)=specificity';
result_matrix(:,4)=error';
result_matrix(:,5)=sensitivity';
save('result_matrix.mat','result_matrix')

clear normal_group_fetures patient_group_fetures normal patient result_matrix
counter=counter+1
end
% end
